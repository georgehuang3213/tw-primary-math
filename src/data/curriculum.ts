...
export const CURRICULUM_UNITS: Unit[] = [
  ...
  },
  {
    ...
    order: 7,
    title: '第七單元 認識時間',
    titleBpmf: '[第|ㄉㄧˋ][七|ㄑㄧ][單|ㄉㄢ][元|ㄩㄢˊ] [認|ㄖㄣˋ][識|ㄕˋ][時|ㄕˊ][間|ㄐㄧㄢ]',
    subtitle: '鐘面1~12、時針與分針、幾點整與幾點半',
    description: '認識鐘面數字1~12與時針短、分針長，學會精確判讀幾點整與幾點半及一日生活作息！',
    icon: '⏰',
    themeColor: 'from-sky-400 to-blue-600',
    manipulativeType: 'clock',
    keyConcepts: ['鐘面結構(1~12數字)', '時針短與分針長', '幾點整(長針指12)', '幾點半(長針指6)'],
    learningGoals: [
      '能辨認鐘面上的 1~12 數字、短針（時針）與長針（分針）走動方向',
      '能精確判讀「幾點整」（長針指在 12，短針指在數字幾就是幾點整）',
      '能精確判讀「幾點半」（長針指在 6，短針指在兩個數字正中間，讀作幾點半）',
      '能將時間判讀與生活作息（如 7點半起床、8點整上學、12點整吃午餐）相結合'
    ],
    story: {
      character: '🦉 貓頭鷹校長的鐘樓城堡',
      scene: '早晨 8 點整，鐘樓發出清脆的叮噹聲。',
      dialogue: '貓頭鷹校長說：「長針指在 12，短針指在 8，就是 8 點整！長針走半圈指在 6 就是幾點半喔！」',
      task: '撥動模擬時鐘指針，撥出早上 8 點整與中午 12 點半！'
    },
    rhyme: {
      title: '認識時鐘兒歌',
      lines: [
    ...
    lessonSteps: [
      {
        stepNum: 1,
        stepTitle: '認識鐘面結構與指針',
        stepDesc: '鐘面上有 1~12 個數字，短針是「時針」（走得慢），長針是「分針」（走得快），順時針方向走。'
      },
      {
        stepNum: 2,
        stepTitle: '判讀整點 — 長針指在12',
        stepDesc: '當長針指在數字 12 上時就是「整點」，這時候看短針指在數字幾就是幾點整（例如短針指在 8 就是 8點整）。'
      },
      {
        stepNum: 3,
        stepTitle: '判讀半點 — 長針指在6',
        stepDesc: '當長針走半圈指在數字 6 上時就是「半點」，短針指在 7 和 8 正中間，代表過了 7 點走了一半，讀作 7點半！'
      }
    ],
    warmup: {
      ...
      options: ['6', '12', '3'],
      answerIndex: 0,
      explanation: '「半點」時長針一定指在 6，短針指在 12 與 1 正中間！'
    }
  },
